...
\def\mytitle{Chapter 2 (Kinematics) Homework A}
\title{\mytitle}
\date{\today}
...
\begin{center}
  \includegraphics[height=4.9in]{2-14.png}
\end{center}
...
For paths A, B, C, and D in Figure~\ref{2-57}, calculate the (i) distance traveled, (ii) the displaccement from start to finish, and (iii) the magnitude of displacement.
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}
      ...
      \fill[red] (2*\m,4*\m) circle (0.1) 
          node[above=5,color=black,fill=white] {\bf C};
      \draw[.->|, red, ultra thick,rounded corners] (2*\m,4*\m) -- ++(8*\m,0)
    ...
    \end{tikzpicture}
    \caption{(Based on OpenStax Fig~2.57)}
    \label{2-57}
  \end{figure}
